...
def main():
    ...
    h_jet_Edep_over_Etruth = ROOT.TH1D("h_edep_over_truth", "E_{dep}^{jet}/E_{truth}^{jet};E_{dep}/E_{truth};Jets", 100, 0, 2)
    h_jet_JES_over_Etruth = ROOT.TH1D("h_jes_over_truth", "E_{JES}^{jet}/E_{truth}^{jet};E_{JES}/E_{truth};Jets", 100, 0, 2)
        ...
        for event in range(len(arrays["mu_pt"])):
            ...
            for j in forward:
                ...
                if matched:
                    ...
                    #ration_jes_over_truth = jet
                    if np.isfinite(ratio):
                        h_jet_Edep_over_Etruth.Fill(ratio)
                        #h_jet_JES_over_Etruth.fill(ratio_jes_over_truth)
                else:
    ...
    h_jet_Edep_over_Etruth.Write(); h_pz["current"].Write(); h_pz["drop_unmatched"].Write(); out.Close()
    print(counts)
